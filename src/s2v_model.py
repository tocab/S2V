...
class s2v(object):
    ...
    def build_inputs(self):
        ...
        def _parse_function(example):
            features = {
                'sentences': tf.VarLenFeature(dtype=tf.int64),
                'sequence_length': tf.VarLenFeature(dtype=tf.int64)
            }
            parsed_features = tf.parse_single_example(example, features)
            return tf.sparse_tensor_to_dense(parsed_features["sentences"]), tf.sparse_tensor_to_dense(
                parsed_features["sequence_length"])
        ...
        if self.mode == "encode":
            ...
            dataset = tf.data.TFRecordDataset(tf.gfile.Glob(FLAGS.input_file_pattern))
            dataset = dataset.map(_parse_function)
            iterator = dataset.make_one_shot_iterator()
            encode_ids, sequence_length = iterator.get_next()
            encode_ids = tf.reshape(encode_ids, [-1, FLAGS.sequence_length])

        self.encode_ids = encode_ids
        self.sequence_length = sequence_length
        self.batch_size = tf.shape(encode_ids)[0]
    ...
    def rnn(self, word_embs, sequence_length, scope, encoder_dim, cell_type="GRU"):
        ...
        for i in range(2):
            with tf.variable_scope("encoder" + names[i]) as scope:
                ...
                if self.config.encoder == "gru":
                    sent_rep = self.rnn(self.encode_emb[i], self.sequence_length, scope, self.config.encoder_dim,
                                        cell_type="GRU")
    ...
    def build_loss(self):
        ...
        scores = tf.matrix_set_diag(scores, tf.zeros(self.batch_size))

        diag_1 = tf.pad(tf.diag(tf.ones(self.batch_size - 2) * 0.5), [[0, 0], [2, 0]])
        diag_2 = tf.pad(tf.diag(tf.ones(self.batch_size - 2) * 0.5), [[0, 0], [0, 2]])

        diag_1 += diag_2
        first_line = tf.reshape(tf.diag(tf.ones(self.batch_size))[1], [1, -1])
        last_line = tf.reshape(tf.diag(tf.ones(self.batch_size))[-2], [1, -1])
        targets = tf.concat([first_line, diag_1, last_line], axis=0)
