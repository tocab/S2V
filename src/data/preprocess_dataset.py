...
def _create_serialized_example(sentences, seq_len, vocab):
    """Helper for creating a serialized Example proto."""
    example = tf.train.Example(features=tf.train.Features(feature={
        "sentences": _int64_feature(sentences),
        "sequence_length": _int64_feature(seq_len)
    }))
...
def _process_input_file(filename, vocab, stats):
    ...
    for sentence_str in tf.gfile.FastGFile(filename):
        ...
        sequence_length.append(min(len(sentence_tokens), FLAGS.max_sentence_length))

        sentence_ids = _sentence_to_ids(sentence_tokens, vocab)
        sentence_ids = np.pad(sentence_ids, [0, FLAGS.max_sentence_length], "constant")[:FLAGS.max_sentence_length]
        sentences.append(sentence_ids)
        stats.update(["sentence_count"])

    sentences = np.reshape(sentences, [-1])
    serialized = _create_serialized_example(sentences, sequence_length, vocab)
...
def main(unused_argv):
    ...
    for filename in input_files:
        dataset.append(_process_input_file(filename, vocab, stats))
